...
const targetDate = new Date("2024-1-1T00:00:00Z");
...
doors.forEach((door, index) => {
  door.addEventListener('click', () => {
    ...
    // 각 날짜에 해당하는 날짜를 계산
    const openDate = new Date(2023, 11, index + 8); // 2023년 12월 1일부터 시작
    ...
    if (now.getTime() > openDate.getTime()) {
      ...
      const checkbox = document.querySelector(`.day-${index + 1}`).querySelector('#checkbox');
